...
int main(int argc, const char **argv) {
...
#else
  {
    ...
    auto *dir_vis = static_cast<uint8_t *>(malloc(width * height));
    for (int32_t p = 0; p < width * height; p++)
      dir_vis[p] = static_cast<uint8_t>((sobel.host_dir[p] + CUDART_PI_F) /
                                        (2.0f * CUDART_PI_F) * 255.0f);
    write_result = stbi_write_png("assets/sobel_dir.png", width, height, 1,
                                  dir_vis, width * 1);
    free(dir_vis);
  }
  ...
  // Write Hysteresis image
  write_result = stbi_write_png("assets/hysteresis.png", width, height, 1,
                                hysteresis.hysteresis_buffer, width * 1);
  if (write_result == 0)
