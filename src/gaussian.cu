...
GaussianResult gaussian_execute(const uint8_t *host_src, const int32_t width,
  ...
  CUDA_THROW_IF_FAILED(
      cudaMalloc(&device_weights_buf, blur_size * blur_size * sizeof(float)));
  CUDA_THROW_IF_FAILED(cudaMemcpy(device_weights_buf, weights,
                                  blur_size * blur_size * sizeof(float),
                                  cudaMemcpyHostToDevice));
#endif
#ifdef GAUSSIAN_SHARED
  CUDA_THROW_IF_FAILED(cudaMemcpyToSymbol(
      device_weights, weights, blur_size * blur_size * sizeof(float)));
#endif
  ...
  gaussian_filter<<<grid_dim, block_dim>>>(
      device_src, width, height, device_weights_buf, BLUR_RADIUS, device_dst);
#endif
#ifdef GAUSSIAN_SHARED
  gaussian_filter<<<grid_dim, block_dim>>>(device_src, width, height,
                                           device_dst);
#endif
  cudaEventRecord(t2);
...
void calculate_gaussian_weights(const uint32_t blur_radius, const float sigma,
  ...
  for (int32_t y = -static_cast<int32_t>(blur_radius);
       y <= static_cast<int32_t>(blur_radius); y++) {
    for (int32_t x = -static_cast<int32_t>(blur_radius);
         x <= static_cast<int32_t>(blur_radius); x++) {
  ...
  for (int32_t y_ = -BLUR_RADIUS; y_ <= BLUR_RADIUS; y_++) {
    for (int32_t x_ = -BLUR_RADIUS; x_ <= BLUR_RADIUS; x_++) {
      const int32_t tile_idx =
          (ty + y_ + BLUR_RADIUS) * tile_size + (tx + x_ + BLUR_RADIUS);
      const uint32_t weight_idx =
          (y_ + BLUR_RADIUS) * (2 * BLUR_RADIUS + 1) + (x_ + BLUR_RADIUS);
