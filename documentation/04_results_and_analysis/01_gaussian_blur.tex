...
Der gemessene Speedup hängt stark vom \texttt{BLUR\_RADIUS} ab
(Abbildung~\ref{fig:gaussian_ncu}). Bei $r=2$ überwiegt der
\texttt{\_\_syncthreads()}-Overhead, sodass die Kernel-Laufzeit bei
nahezu identischem SM Throughput nur marginal sinkt. Bei $r=9$ kehrt
sich das Verhältnis um, da die $361$ Faltungsoperationen pro Pixel den
redundanten DRAM-Traffic zum dominanten Faktor machen, wodurch Shared
Memory einen Speedup von $3.0\times$ bei deutlich gesteigertem SM
Throughput erzielt.
...
(S.~\pageref{app:gaussian_results}) aufgeführt.
